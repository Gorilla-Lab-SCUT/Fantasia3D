...
Answer: Yes, it can. Just Original SDS loss combined with negative prompt and strategy 2 proposed in Fantasia3D is **enough** to address the oversaturated and over-smoothing problem. The appearance of the DMTet-based gallery in [Sweetdreamer](https://sweetdreamer3d.github.io/dmtet-based-gallery_0.html) is generated by the code of appearance modeling in Fantasia3D. You can see that all the results are highly detailed and do have not the problem of oversaturation and over-smooth. I think the key is the disentangled representation and the adoption of the negative prompt and strategy 2. The recommended negative prompt is "shadow, oversaturated, low quality, unrealistic". The recommended positive prompt is "a DSLR photo of ...".

***Q8***: *Can Fantasia3D support single image to high-quality 3D assets with high-detail geometry and photorealistic appearance?*

Answer: Yes, it can. Firstly you can use the shape-e, point-e, or any other single image to 3D methods, such as magic123, one-2-3-45, and DreamGaussian, etc. to generate a coarse mesh. Subsequently, use the coarse mesh as the DMTet initialization in user-guided generation in Fantasia3D to attain a high detail geometry. Lastly, use the result of user-guided generation to attain photorealistic texture in appearance modeling.

***Q9***: *What is the difference between the official code and the reproduced version of threestudio?*
