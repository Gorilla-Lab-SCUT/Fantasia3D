...
- **(geometry modeling) Rotate the object.** Rotating the object according to the actual situation can alleviate janus-problem or help the network in mode-seeking. For example, when generating a human head statue, rotate the initialized ellipse around the x-axis by some angle to match the situation where the back of the person's head has some curvature.
